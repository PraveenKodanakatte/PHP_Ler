...
<html>
  ...
  <br>
    Hobbies:<input type="checkbox" name="hobbies" value="Cricket"
    >
    ...
    if($result-> num_rows > 0){ $i=1;
      while($row = $result-> fetch_assoc()){
          ...
          echo "
            ...
            <td><a href='update.php?up=$row[id]&fn=$row[Name]&Mb=$row[Mobile]&Ag=$row[Age]&gn=$row[gender]&ct=$row[city]&cm=$row[comment]&hb=$row[hobbies]'>edit</td>
            </tr>
            ";
      }
    // connection build
